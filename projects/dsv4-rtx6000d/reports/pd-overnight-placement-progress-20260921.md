...
**同48八卡、GovReport C32下，普通双TP4为1026.74±1.52，修正UCX可达性的1P1D为911.15±1.36 output tok/s，PD低11.26%。** 两模式正式三轮无已知编译事件，工作量通过。跨节点普通为1028.71±3.36、1P1D为914.23±1.98 tok/s，PD低11.13%。位置四组已完成；16K、DP/DSpark和配比继续执行，不能据此结束PD研究。
...
| A3 | 普通2×4，总8 | 跨46/48，后四卡 | 1028.71±3.36 | 3.960 | 27.107 |
| A4 | P1×4+D1×4，总8 | 跨46/48，后四卡 | 914.23±1.98 | 4.243 | 28.843 |
...
实际服务/代理命令及每轮协议/计数见[执行快照](../data/pd-overnight-placement-progress-20260921.json)，均直接从运行产物导出。完整原始产物仅45本机：`experiments/dsv4-pd-overnight/results/{A1,A2,A2T,A3,A4}-01/`，含resolved/command/preflight/affinity、worker-warmup、server/proxy日志、每轮raw/metrics/计数、资源时间序列与冻结检查；`reports/`保存资源复核和缓存继承哈希。全部位置case收尾均无代码配置变动、无清理错误。

下一步与预算见[夜间计划](pd-overnight-plan-20260921.md)。本页为约04:20的位置阶段记录，后续完整报告优先于此进度页。

跨节点普通A3三轮1029.72/1031.45/1024.96，PD A4三轮913.63/912.61/916.44 tok/s，均无已知事件；原始精度以执行快照为准。A4的KV计数与A2T同量，失败/过期/抢占及D整段prefill重算均0。位置变化下普通均值差0.19%、PD差0.34%，目前没有证据将约11%的普通/PD差距主要归为跨节点位置；A2T的网络配置变化及NIC路径限制仍适用。

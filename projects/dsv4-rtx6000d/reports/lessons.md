...
TP8 正式 18 次结果全部通过工作量与 gate 验收。vLLM C32 首次尝试完成请求，但因 16 条 TileLang 日志被拒绝；重新按既定上限预热后通过，未按速度挑样本。有效测量未观察到 OOM 或请求超时。
...
四节点TP4对照补充了两点：CPU/GPU对称绑定应同时核对物理核、客户端位置、内存约束和后台负载；编译日志静默时仍可能产生编译产物，进程内cache miss又不一定代表重新编译。47节点保留了这两类反例并在最终有效窗口确认事件与产物变化为零，具体见[节点47报告](tp4-node47.md)。这些本地观察规则尚未作为公共runner能力发布。

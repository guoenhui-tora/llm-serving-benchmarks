...
**TP2×PP4 已补齐 C32/C64 各三次，完整 PASS；本节点四种部署现均有完整批次，共 24 个正式整机样本。** 本次将 C64 预热上限由五轮提高至十轮，实际第八轮达到连续两轮安静，之后三次测量均一次通过；没有改动服务参数、正式工作量或 gate。

| 配置 | 正式采用批次 | C32 输出 tok/s，均值 ± 样本标准差（CV） | C64 输出 tok/s，均值 ± 样本标准差（CV） |
| --- | --- | ---: | ---: |
| 双 TP4 | baseline-02 | 1095.66 ± 1.99（0.18%） | 1433.82 ± 0.93（0.07%） |
| TP8 | candidates-01 | 611.92 ± 0.91（0.15%） | 681.52 ± 0.69（0.10%） |
| TP4×PP2 | candidates-01 | 681.99 ± 20.09（2.95%） | 902.79 ± 3.83（0.42%） |
| TP2×PP4 | tp2-pp4-03 | 1105.42 ± 12.86（1.16%） | 1472.15 ± 6.18（0.42%） |

TP2×PP4 的吞吐均值在 C32/C64 分别比双 TP4 高 **0.89%／2.67%**，差距较小，预热预算和缓存历史不同，不能据三次样本断言稳健的最优部署。TP2×PP4 的 C64 P95 ITL 均值为 33.52 ms，高于双 TP4 的 22.56 ms，也不是所有延迟指标都更好。TP4×PP2 首批 C32 的波动限制仍保留。

[当前 CSV](../data/node8-node48.csv)保留此前 42 行原值，追加 TP2×PP4 的 12 行，合计 **54 行：24 个 node、24 个 replica-0、6 个 replica-1**。仅筛选 `scope=node` 得到 **4608 成功、0 失败，输入 37,748,736、输出 4,718,592 tokens**。每个配置的 C32/C64 都来自其同一次启动，不拼接旧部分样本；01/02 批次失败的 TP2×PP4 和 01 批次失败的双 TP4 仍仅作历史证据。

## 补测 03：预热预算调整与完整结果

用户在复核 02 批次 `28/24/0/14/0` 的预热序列后，明确授权增加一次 TP2×PP4 批次，将 C64 上限改为十轮。该旧序列说明五轮结束时仅有一轮安静，不证明 C64 无法稳定测量。

本次仅在 `gpu-6000d-48`（`10.90.1.48`）启动 TP2×PP4 一次，北京时间 **2026-09-18 10:29–11:43**，约 **74.3 分钟**；其他三个配置未重跑。执行 commit 为 `534d4d3`，开始时工作树干净。固定镜像、模型/tokenizer、runner 源码指纹、recipe、客户端、资源绑定和缓存目录均与 02 批次一致，已逐字段核验；唯一负载配置差异为本地 C64 的 `max_warmup_rounds: 5 → 10`。

C32：8192 输入／1024 输出、并发 32，预热 64 请求、正式 128 请求，最多五轮预热。C64：同长度、并发 64，预热和正式各 256 请求，最多十轮预热。两者仍各三次、连续两轮无已知事件、最多两次测量尝试，客户端单阶段超时 7200 秒。达到安静条件立即测量，没有固定跑满十轮。所有服务优化、旧 JIT 缓存及 warning 保留；未修改公共 runner、gate、正式 recipe 或镜像源码。

| 阶段 | 预热事件数序列 | 首次出现的 mHC 分块键 | 正式测量 |
| --- | --- | --- | --- |
| C32 第一次 | 24/0/0 | split52；另有 Triton monitor 事件 | 一次通过 |
| C32 第二、三次 | 各 0/0 | 无新增事件 | 各一次通过 |
| C64 第一次 | **28/10/10/8/0/14/0/0** | 按轮次：11/12/19；13；15；17；无；14；无；无 | 第八轮后一次通过 |
| C64 第二、三次 | 各 0/0 | 无新增事件 | 各一次通过 |

C64 第四轮的 split17 为新的实际编译形状。PP0 两个 TP worker 的普通 `mhc_pre_big_fuse_with_norm_tilelang` 在内部 UTC **03:03:20–03:03:30/31** 编译，这四条 begin/end 到 **03:10:54** 才进入 Docker 日志，属于第六轮收到的延迟输出。第六轮同时有 split14 的 10 条实时 monitor 事件，所以即使精确修正时间归属，该轮也不应判安静。第七、八轮真正无已知事件后才进入测量，六次正式窗口均无事件、无拒绝尝试。

TileLang 缓存由 **110 增至 115 个文件**：一个新 kernel 目录的四个文件和一个 cubin；原 110 个文件的大小、mtime 和 SHA256 全部保持不变。它与真实编译日志相互印证；其他首次键警告不能单独证明 CPU 重编译。此次实测支持“额外计时外预热可解决本批次形状覆盖不足”，不保证任意调度形状或长期运行都不再编译。

| 并发 | 三次输出 tok/s | Mean TTFT | Mean TPOT | P95 ITL | P95 E2EL |
| --- | --- | ---: | ---: | ---: | ---: |
| C32 | 1113.756984 / 1111.886902 / 1090.608193 | 3494.49 | 25.54 | 22.96 | 33260.86 |
| C64 | 1465.659502 / 1472.813612 / 1477.966212 | 5105.34 | 38.48 | 33.52 | 51166.47 |

延迟单位为 **ms**，各列为三次对应指标的均值，P95 均值不是三次合并请求后的 P95。保留较慢的第三次 C32；CV 为 1.16%，未触发 3% 波动观察阈值，也未据此声称完全收敛。

本批次六次正式测量合计 **1152 成功、0 失败，输入 9,437,184／输出 1,179,648 tokens**。共 **25 个预热/测量阶段、4672 请求**；各阶段实际数量、逐请求 8192/1024、索引覆盖、客户端/服务绑定均独立核验。整机统计仍采用官方同步窗口；单副本的 node 与 replica-0 行不能重复相加。

运行区间内 **148 个独立 30 秒进程采样**覆盖资源背景，DOCA 为 **15.998–16.003 核**，未发现 DOCA 和本次 Docker cgroup 之外超过 0.5 核的单进程采样，也未发现矿工异常。六个正式窗口的整机 CPU 平均忙碌率为 **18.81–19.07%**，最高采样 **19.73%**；GPU 最高 **62℃**、最低采样 SM 频率 **2415 MHz**，其他后台合计峰值 **0.118 核**。DOCA 可迁移，低频采样不能排除短时干扰，因此 CSV 标记 `accepted-background`，不称 CPU 独占。

完整 required/kernel 日志检查 PASS，142 条 warning 条目保留。实际 TP2/PP4/DP1、EP off、Graph/backend 与原 recipe 一致；KV 日志仍为 **51.68 GiB／475,128 逻辑 tokens**。C32 初期保存的 `/metrics` 快照累计抢占为 0，完整日志未见抢占、OOM 或 CUDA error；未取得结束前的累计抢占快照，不将其描述为全程指标证明。

最终本批次容器已清理，八卡显存 0 MiB、无 GPU 计算进程，独立监控已停止；模型、镜像、旧产物和缓存保留。未访问其他节点，未 push。

本地复现入口和完整证据**仅在 48 本机**：

- 新 run-root：`experiments/dsv4-node8-node48/results/tp2-pp4-03/`。
- 新 campaign：工作区 `configs/campaigns/48-dsv4-node8-tp2-pp4-warmup-03.yaml`，仅保留 TP2×PP4，引用原 recipe/target/runtime/client/model 与 C32 workload；本地 `workloads/node8-c64-full-warmup-03.yaml` 相对正式 C64 仅改 `warmup_requests: 256`、`max_warmup_rounds: 10`，其余字段/ID 不变。
- 工作区 `reports/jit-followup-03/`：用户授权与冻结预算 `plan.md`、preflight/plan、完整控制台日志、逐阶段/样本审计、进程采样、缓存前后 SHA256、固定身份与最终清理证据。

复现时复制公共 configs 到新工作区，按上述两个预热字段重建本地 workload/campaign，先 validate、plan、preflight，再使用新的 run-root。实际命令如下，不覆盖本次产物：

```bash
./bench run experiments/dsv4-node8-node48/configs/campaigns/48-dsv4-node8-tp2-pp4-warmup-03.yaml \
  --case tp2-pp4 --run-root experiments/dsv4-node8-node48/results/tp2-pp4-03
```

补测 03 自审：25 个阶段的实际工作量、分片和绑定，六次正式样本的连续安静条件和吞吐重算，旧 CSV 42 行原值及新增 12 行导出一致性，文档数字/相对链接，固定身份、旧缓存哈希和资源清理均已核对；本地与公共配置 validate/plan 及相关 CSV 导出回归检查通过。

## 补测 02 历史记录：五轮预算下的结果

以下“本次”均指 02 批次，保留其实际预算和失败证据，不代表当前仍有未完成配置。该批次于北京时间 2026-09-18 01:57–03:39 依次测双 TP4、TP2×PP4：双 TP4 完整通过；TP2×PP4 仅 C32 三次通过，1110.288935／1114.999265／1105.339461 tok/s，均值 1110.21 ± 4.83、CV 0.44%，未进入正式 CSV。02 批次归档时 CSV 共 42 行、18 个 node 样本；当前累计数据以上文为准。
...
02 批次结束时提出的未实施建议（03 已通过增加预热预算补齐）：优先设计一次确定的计时外形状覆盖，逐 worker 核对两个 mHC kernel 的 split10–15 等实际缺失键，再执行原协议的安静轮和测量。单纯追加相同请求不能保证覆盖；本次序列至少还需第六轮才可能形成两轮安静，但这不是第六轮一定通过的证据。本节点未追加启动或无限加轮。若需服务内部 warmup 或改进 monitor 的缓存命中分类／日志发生时间归属，应统一改动并以以上真实编译、缓存复用和延迟反例验证，不由本节点私改 runner、gate 或镜像。建议尚未实测。
...
以下保留首次归档的执行范围、结果与失败证据。其“本轮”和 CSV 24 行等数字均指首批快照；当前累计范围及 CSV 54 行以上文为准。首批两个失败配置的 C32 均未纳入补测结果。

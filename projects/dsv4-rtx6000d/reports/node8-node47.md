...
9月18日凌晨向两个 EP off 候选各追加一个批次：增加本地计时外C64覆盖预热后，TP2×DP4完成C32/C64各三次；TP4×DP2仍出现新的MHC形状和真实编译，未进入正式测量。未修改公共runner、gate或正式recipe。新增结果和比较限制见下文。随后经用户授权，于10:32–12:07仅向TP4×DP2 EP off追加一个扩充预热上限的批次：覆盖预热通过，C32完成三次，但C64两次测量均出现worker首次使用事件而被拒绝；完整日志无TileLang实际编译记录，缓存工件未变。正式CSV仍为24个整机样本，最新部分结果及带事件的C64诊断值见文末；本节点测试已收尾。
...
本节点测试在此收尾，保留以下两次C64实测值并明确其资格：**测量窗口内出现JIT monitor首次使用事件，未通过原gate，不是可信的无事件稳态结果**。两行是同一重复的两次尝试，不是两份通过验收的独立重复；不计算正式均值、标准差或CV，不进入正式CSV、性能表或部署排名。它们反映包含首次使用开销的本次观测，事件对吞吐及延迟的影响幅度未量化；既不能当作稳态值，也不能保证是稳态吞吐下界。

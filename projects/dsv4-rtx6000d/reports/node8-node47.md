...
下表保留首批结果；9月18日追加批次见后文“JIT诊断与补测”。当前 TP2×DP4、EP off 已有完整通过的新批次；TP4×DP2、EP off 后续仅取得独立C64三次，仍无同启动的完整C32/C64批次。
...
[逐次 CSV](../data/node8-node47.csv)由[统一导出器](../../../scripts/export_samples.py)生成，筛选已完成授权范围且通过 gate 的 `purpose=performance`（最新仅C64独立批次单列）：首批18个整机样本加TP2×DP4补测6个，再加用户单独授权的TP4×DP2 C64独立批次3个，共27行整机样本、27行 `replica-0`、6行 `replica-1`，合计60行。只对 `scope=node` 求和，共5376成功、0失败，输入44,040,192、输出5,505,024 tokens。首批两份 FAIL 配置的6个 C32 trial 仍单列，不与新批次拼接；追加的诊断预热和验证也不进入CSV。首批共24个已完成且通过 trial gate 的正式测量、1次被拒绝的正式尝试，另外6次 C64 因预热阻塞未执行；凌晨TP2×DP4补测新增6次正式测量，无拒绝尝试；之后独立C64补测新增3次有效测量，并保留1次被拒绝尝试。
...
下表包含完整C32/C64 PASS批次（TP2×DP4 EP off使用凌晨补测），并单列后来授权的TP4×DP2 EP off独立C64批次，吞吐为三次均值±样本标准差；延迟为三次对应指标的均值，单位均为 ms。P95列不是把三次请求再次合并后的分位数。完整 mean/P95 TTFT、TPOT、ITL、E2EL与各侧窗口见 CSV。
...
| TP4×DP2，EP off，独立C64补测 | 64 | 878.47 ± 26.48 | 3.01% | 12265.03 | 41016.31 | 59.12 | 30.86 | 106296.16 |
...
联合审查覆盖11轮长预热、10轮正式流程预热、4次测量尝试，共25个完成阶段：**5120成功、0失败，输入41,943,040、输出5,242,880 tokens**。逐请求、绑定、CPU/GPU和cgroup时间窗口均核对通过。仅三份有效C64样本进入正式CSV，标为accepted-background；长预热、正式预热、拒绝尝试及旧C32均不计入。

...
### 4. 16K、C128追加结果：12卡2P1D低于普通6.24%

**同一批512条真实16K输入／1K输出，全系统C128，DSpark off。** 每服务TP4、4卡；双方各一次启动，一轮完整预热＋三轮quick，正式均0已知编译事件。

| 部署（双方均12卡） | output tok/s | Mean TTFT s | Mean TPOT ms | Mean E2EL s | P95 E2EL s |
| --- | ---: | ---: | ---: | ---: | ---: |
| 普通：3个完整推理服务 | 1358.54±0.56 | 12.14 | 81.76 | 95.77 | 145.27 |
| PD：2个P＋1个D | 1273.81±0.18 | 50.39 | 43.39 | 94.78 | 123.61 |

**PD吞吐低6.24%，首token平均等待约为普通4.15倍；生成更快，整请求P95低14.91%，平均整请求延迟只低1.04%。** 以吞吐为主要目标，当前C128配置不推荐替代普通。此前C64／N256的+2.88%不能外推到这里，且本轮N512与旧请求集大小不同，不能把跨批变化全归因于并发。

中段两个P各约30条等待，D约61.6条运行、等待不到1条；P平均排队45–46秒，D约0.117秒。D采样最大运行63，未见持续触及64上限。证据优先指向P供给／调度效率，尚未定位唯一内核瓶颈，不直接加D或铺更多比例。全部KV真实传输验收通过。

**本轮另修复了代理默认100条上游连接的限制**（`2511f04`，12项HTTP／PD回归通过）；以上C128双方均使用修复后代理。旧C128预热中止并留证；旧C64已完成单元仅保留为诊断，不并入新配对。为保持3小时与请求预算，PD C64补测未启动。性能执行及清理约123分钟，原计划与调整均保留。

16K下的DSpark及TP2×DP2等其他拓扑尚未实测。详见[结果、瓶颈与复现报告](reports/pd-16k-concurrency-20260921.md)、[逐轮数据](data/pd-16k-concurrency-20260921.csv)和[原计划](reports/pd-16k-short-plan-20260921.md)。

### 此前两组正向候选的延迟代价

TTFT是首token等待时间，TPOT是首token后平均每token耗时。下表是此前C64／C32配对的三轮均值，与新增C128结果分开看；列顺序均为普通→PD。

| 配对 | 普通Mean TTFT | PD Mean TTFT | 普通Mean TPOT | PD Mean TPOT |
| --- | ---: | ---: | ---: | ---: |
| 16K／C64／N256／12卡：3个TP4完整推理服务 → 2P1D（均TP4） | 8.195 s | 9.480 s（+15.68%） | 46.401 ms | 41.627 ms（−10.29%） |
| 8K／C32／N128／8卡：2个TP2×DP2 EP on完整推理服务 → 1P1D（同拓扑，双方DSpark K5，修复后） | 1.763 s | 3.985 s（+126.03%） | 19.491 ms | 15.883 ms（−18.51%） |
...
后续仍以**16K输入／1K输出，同资源下的吞吐与延迟收益**为主线。C64小幅正向而C128落后，说明既有收益不能跨负载外推；暂不扩大TP4比例矩阵。若继续，优先有界定位P供给／调度效率，再决定一个优化候选。普通混合推理的最佳拓扑不能直接当作纯P或纯D最优。

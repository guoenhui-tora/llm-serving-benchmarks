...
**TP4扩展的quick证据已覆盖C64多服务：** 8K/16K三副本普通、1P2D、2P1D，以及8K/16K四副本普通和2P2D，共10组、30轮正式测量均0已知事件（每组先完整N256 HTTP预热）。部分普通组HTTP预热仍出现24/64条top-k索引事件，所以不能省去该步骤。**但8K 2P2D即使三轮0事件，吞吐仍由1664.92升至1817.95，CV4.79%、TTFT CV约19.5%；预热阶段另有NIXL传输长尾。JIT覆盖不能替代性能收敛与传输检查。** 这里只确认上述十组。**新增16K六副本普通（24卡）正式仍有4/4/0条top-k索引事件**，即使吞吐CV仅0.014%，也不能称三轮无JIT基线。其完整HTTP预热已有88条事件，说明增加服务后仍可能遗漏各进程实际调度分派；不追加轮次或只选最后一轮。4P2D按含事件quick参照解释，详见[配比报告](reports/pd-ratios-c64-20260921.md)。

...
在 46/47/48 各八卡固定四个四卡 P、两个四卡 D（各 TP2×DP2、EP on、DSpark K5），真实 GovReport 精确 16K/1024、全局 C16～C144 步长 16 以及追加 C160 均完成完整预热与三轮正式测量。整机 output tok/s：C64 **2680.39±14.17**、C128 **3930.44±18.39**、C144 **4120.98±8.80**；C160 为 **4263.74±35.61**，但 Mean TTFT **11.898 s** 首次超过 10 s 停扫线。按**相同全局并发/GPU**对齐历史，12卡2P1D/C64→本批24卡4P2D/C128 吞吐约2.02倍，16卡3P1D/C96→本批C144约1.60倍；旧组镜像、数据和容量不同，这只是方向性历史比例，不证明 4P2D 对 16K 最优。同资源 `2×(2P1D)` 未测。双四卡 D 在八卡 decode-only 对照胜出，因此按预定规则没有运行单八卡 D 的 PD 拓扑。[完整逐档结果、历史扩容参照与复现方法](reports/pd-24gpu-16k-4p2d-20260925.md)。

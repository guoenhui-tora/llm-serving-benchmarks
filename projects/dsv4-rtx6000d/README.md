...
**继续选择 vLLM 0.29.0、关闭 FlashInfer autotune。在本轮 8K 输入 / 1K 输出、C32 下，八卡优先优化 TP2×PP4，四卡优先考虑 TP2×PP2。** 前者在46达到 **1099.61 ± 19.98 tok/s**，后者在48达到 **762.92 ± 17.85 tok/s**，均为三次重复。它们各自优于同节点参照，但尚未在同一节点直接比较，也未验证其他并发或长期稳定性。
...
**八卡最值得继续的是 TP2×PP4。** 46相对本机 TP8，吞吐提高80.34%，mean TTFT/TPOT分别降低64.61%/39.90%；相对四卡 TP4，整机吞吐提高54.44%，但用卡数翻倍，不能称为卡效也更高。TP4×PP2仅比 TP8快9.34%，mean TPOT还略高，因此不是只要开启 PP 就一定大幅提速。
...
以下是本轮结果支持的优先顺序，尚未执行，不代表已安排新一轮全矩阵。

- [ ] **复核八卡 TP2×PP4。** 以prefill8192为起点，在另一节点先校准同机TP8参照再测PP候选，各三次；重点确认约1100 tok/s的收益可复现，持续观察后台CPU负载。
- [ ] **八卡 `--numa-bind` 对照。** 在同一节点、同一优选拓扑上比较默认不绑定与开启该参数，保持负载和客户端条件一致，各三次；观察吞吐、TTFT/TPOT及波动。固定镜像默认关闭，需核实worker近端CPU/内存、rank映射和权限；若使用Docker cpuset，两层允许范围必须兼容。
- [ ] **正常背景下复核45的DP＋EP。** 先确认异常服务在整个测量窗口不再恢复，再比较本机TP8与DP2 off/on，不与本轮受干扰样本混合。尚不能预测它与TP2×PP4的同机排序。
- [ ] **保留四卡 TP2×PP2路径。** 若关注少卡部署或后续阶段拆分，再复核吞吐顺序下降及延迟取舍；本轮单服务结果不能直接推算双实例总吞吐或PD性能。

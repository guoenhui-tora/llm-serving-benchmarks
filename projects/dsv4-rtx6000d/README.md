...
结论适用于当前同机双服务及宿主后台CPU负载，较慢引擎存在另一侧空闲的测量尾段；不是独占TP4或TP8基线。预热曾发现无编译日志但吞吐未收敛，以及真实Triton/TileLang JIT，均在正式测量前排除。详见[完整报告、参数与复现入口](reports/tp4-48-aligned.md)、[逐次数据](data/tp4-48-samples.csv)。45、46、47 节点结果见下方校准进度。
...
- [x] **gpu-6000d-46**：两引擎各三次有效测量；vLLM / SGLang 为 **705.14 / 565.00 tok/s**，吞吐 CV **0.61% / 0.18%**。见 [报告](reports/tp4-node46.md)、[逐次数据](data/tp4-node46-samples.csv)；CPU 背景负载限制及仅本机保存的原始产物见报告。
- [x] **gpu-6000d-47**：两引擎各三次有效测量完成；vLLM **676.05 ± 3.22 tok/s**、SGLang **585.83 ± 2.61 tok/s**，吞吐 CV **0.48% / 0.45%**。见 [TP4 对齐报告](reports/tp4-47-comparison.md)、[逐次数据](data/tp4-47-samples.csv)。
...
四节点共完成 **24 次正式测量**，每节点三对，不含预热及失败尝试。现有 GPU 锁和占用检查按选中 GPU 生效，可使用不重叠的卡组；但 runner 尚无双服务的测量同步机制，target 也未提供 CPU 绑定字段。执行前需准备对应配置与协调方式，不能直接并发启动原八卡 campaign。45 节点使用项目内 [固定协议协调脚本](scripts/paired_run.py)，48 节点使用其[报告](reports/tp4-48-aligned.md)记录的本地协调脚本，分别完成 CPU 绑定、双客户端计时屏障和证据保存。45 节点脚本针对该节点的既定资源分工，不能直接替代 48 节点协议；公共 runner 的能力边界未改变。

47 节点使用本机协调脚本完成独立 CPU/NUMA 绑定、顺序预热及计时前同步；本次仅归档报告和逐次 CSV，执行脚本、配置和原始日志的位置见[47 节点报告](reports/tp4-47-comparison.md)。

47 节点实际 KV 为 vLLM **32.73 GiB/卡**、SGLang **约 32.43 GiB/卡**（按实际池尺寸重建），差约 0.93%。本轮补充了持久化编译产物检查，并精确区分固定 vLLM 镜像的 TileLang 进程 cache-miss warning；全部 6 个有效测量窗口的编译事件、产物变化和该 warning 都为 0。最终每对仅一次测量尝试，保留两轮观测诊断，详见报告。

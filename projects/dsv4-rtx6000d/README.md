...
三类已知缺口均已修复。最新验证覆盖DSpark 1P1D、DSpark普通双服务及16K普通六TP4：**40个worker检查通过，9轮正式测量全部0已知编译事件**。新增输入内核覆盖每worker约0.14–0.24秒，适用已继承缓存的本轮配置，不代表冷启动编译成本。

1. **保留缓存。** 新recipe需要新缓存目录时，完整继承旧缓存并核对内容，不清JIT缓存。
...
3. **单独保存完整HTTP预热，再测三轮quick。** C32用完整128条，C64用完整256条；默认2C预热不能替代。本轮普通DSpark预热仍有24条其他内核事件，正式三轮才为0。
4. **同时看事件和波动。** 0事件不保证性能稳定；保留全部正式轮，不挑最快、不靠追加轮次消除标签。改变拓扑、K、预算或负载后，重新核对覆盖与容量。

补丁不改变forward、权重、精度或事件识别规则。原mHC问题见[定向预热报告](reports/pd-targeted-warmup-20260921.md)，最新适用范围、回归测试和逐轮证据见[JIT修复报告](reports/input-kernel-warmup-20260921.md)。

## 跨节点PD研究

**这里的“普通推理”是多个完整推理副本：每个服务独立完成prefill和decode，代理把请求分给这些服务。PD则把同一请求先交给P，再将KV传给D生成输出。** 双方都按全系统吞吐计时，包含代理；PD还包含P处理和KV传输。

已完成性能测量的每种PD方案都有**同GPU集合、同数据、同总并发**的普通对照。下面直接列出双方部署及结果；P预算补测复用了对应普通结果，并非每次都重跑普通。每个服务占4卡，P/D数量指服务数，不指节点数。例如4P2D是6服务×4卡＝24卡，使用三台GPU节点，45负责控制。

所有表格使用真实GovReport：8K为近8K完整报告，16K为精确16384 tokens的真实报告前缀；输出固定1024 tokens。C是**全系统并发**。每组一轮完整HTTP预热＋三轮quick，吞吐单位为**output tok/s，均值±样本SD**，“变化”均为PD相对该行普通的变化。固定vLLM 0.29.0、NVFP4权重与FP8 KV；这些结果不与历史random数据混算。

### 1. 八卡TP4：1P1D均落后于两个完整推理服务
...
**本轮TP4 1P1D均落后于两个完整推理服务。** 同节点和跨节点都低约11%，没有证据把差距主要归因于跨节点通信；但同节点组也有NIC传输，不能视作零通信开销对照。

P预算控制每次调度可处理的token量。8192→16384在8K输入下使PD吞吐下降1.65%，在16K下提高2.69%，仍不足以超过普通。16K原1P1D的P平均排队约23.10秒，这支持继续测试增加P的2P1D，而非直接增加D。
...
**只有16K、12卡的2P1D取得小幅吞吐增益，扩大到24卡4P2D没有延续这个优势。** 这里总并发始终是C64，不能将结果解释为提高并发后的扩展上限。16K 1P2D的P中段平均等待约50条、两个D各仅5–6条运行，说明该配置更需要改善P供给。
...
### 3. 八卡TP2×DP2 EP on：DSpark K5下1P1D小幅领先
...
TTFT是首token等待时间，TPOT是首token后平均每token耗时。下表仍是上述配对的三轮均值，列顺序均为普通→PD。

| 配对 | 普通Mean TTFT | PD Mean TTFT | 普通Mean TPOT | PD Mean TPOT |
| --- | ---: | ---: | ---: | ---: |
| 16K／12卡：3个TP4完整推理服务 → 2P1D（均TP4） | 8.195 s | 9.480 s（+15.68%） | 46.401 ms | 41.627 ms（−10.29%） |
| 8K／8卡：2个TP2×DP2 EP on完整推理服务 → 1P1D（同拓扑，双方DSpark K5，修复后） | 1.763 s | 3.985 s（+126.03%） | 19.491 ms | 15.883 ms（−18.51%） |

两组都是生成更快、首token更慢，平均请求总延迟分别下降6.47%和6.77%。因此不能仅凭吞吐决定是否采用PD。24卡4P2D还有另一种取舍：Mean TTFT下降21.31%，P95 TTFT却增加36.67%，且吞吐下降；平均值不能替代尾延迟判断。这里P95取各轮P95的均值，并非合并请求分位数。

### 已确认边界与后续优先级

已验证固定镜像NIXL 1.3.2下的DSV4 FP8、packed MLA、滑动窗口及上述8K DSpark路径：P处理全部输入，D有真实KV字节与逐TP传输，远端命中全部输入，新增prefill计算为0。联合检查避免了D重算输入或“HTTP成功但传输失败”的误判；完整生成质量与逐token数值等价尚未验证。部署过程和时序图见[跨节点协同流程](reports/pd-cross-node-flow.md)。

后续以**16K输入／1K输出，同资源下的吞吐与延迟收益**为主线。当前12卡TP4 2P1D的+2.88%只是候选依据：8K同配比反而落后18.45%，输入长度、并发和DSpark都可能改变P/D平衡。普通混合推理的最佳拓扑不能直接当作纯P或纯D最优。

### 短期计划：约3小时，以16K为主（执行中）

**执行调整（09-21）：** 普通C64三轮已完成；C128预热发现代理默认100条上游连接的限制，已中止并清理服务后修复，HTTP回归12项通过。剩余预算优先完成修复后C128的普通／PD配对，暂不启动PD C64；保留原C64结果与中断证据，不混合代理版本。新增普通部署仅用于验证该明确阻塞的修复，总时间与请求预算不增加。下文保留原四单元计划作为对照。
...
- 固定`vllm/vllm-openai:v0.29.0`及image ID `sha256:c2914767605584b6d8f45686b82de173ecc99e781897aa3d0a66dacd72c51ae1`、原NVFP4模型与FP8 KV；保留全部JIT缓存，接入上文启动覆盖。P预算16384，D／普通8192，上下文32768；prefix cache与autotune关闭。两档保持相同服务配置。
...
原始配置、逐轮结果、失败日志和命令快照拟保存于45本机`experiments/dsv4-pd-16k-short/`，运行时显式指定其`results/`下全新`--run-root`；精选报告与对照表再归档回本项目。该工作区已建立；原计划与执行调整分开记录，完成后按实际结果更新状态。

数据、完整启动命令、失败记录与原始产物入口见[夜间总览](reports/pd-overnight-results-20260921.md)、[执行计划](reports/pd-overnight-plan-20260921.md)及[JIT修复报告](reports/input-kernel-warmup-20260921.md)。历史夜间16K使用1024条真实报告前缀储备池，C32取前128条、C64取前256条，见[数据准备证据](data/govreport-16k-1024-preparation.json)。历史普通双TP4的1033结果及早期682失真参照，见[分流校准](reports/pd-calibration-20260921.md)；不与本轮数据混算。
...
PD候选与未验证项见[上述优先级](#已确认边界与后续优先级)。普通双TP2×PP2、双TP2×DP2 EP on的波动，以及vLLM `--numa-bind`与Docker cpuset的worker/页面分布比较，保留为独立方向；历史PARTIAL不自动补测。

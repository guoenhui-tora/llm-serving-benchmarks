...
结论适用于当前同机双服务及宿主后台CPU负载，较慢引擎存在另一侧空闲的测量尾段；不是独占TP4或TP8基线。预热曾发现无编译日志但吞吐未收敛，以及真实Triton/TileLang JIT，均在正式测量前排除。详见[完整报告、参数与复现入口](reports/tp4-48-aligned.md)、[逐次数据](data/tp4-48-samples.csv)。45、46 节点结果见下方校准进度，47 节点仍待执行。
...
## TP4 节点校准计划（45、46、48 已完成，47 待执行）

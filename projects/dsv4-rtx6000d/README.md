...
在 46/47/48 各八卡固定四个四卡 P、两个四卡 D（各 TP2×DP2、EP on、DSpark K5），真实 GovReport 精确 16K/1024、全局 C16～C144 步长 16 以及追加 C160 均完成完整预热与三轮正式测量。整机 output tok/s：C64 **2680.39±14.17**、C128 **3930.44±18.39**、C144 **4120.98±8.80**；C160 为 **4263.74±35.61**，但 Mean TTFT **11.898 s** 首次超过 10 s 停扫线，未继续增加并发。双四卡 D 在八卡 decode-only 对照胜出，因此按预定规则没有运行单八卡 D 的 PD 拓扑；不能据此推断单八卡 D 的 PD 性能。[完整逐档结果、命令快照与复现方法](reports/pd-24gpu-16k-4p2d-20260925.md)。

...
**扩容时还要重新核对 CUDA Graph：0 JIT 不代表 Graph 覆盖充分。** 本次K5把每DP引擎容量32改成48，却沿用192-token捕获上限，导致大batch形状未覆盖；补齐到288后，D96/C80的8K单轮TPOT由35.86降至10.06ms。Graph尺寸按执行token数计算，详见下方[decode扫描与Graph修正](reports/decode-graph-coverage-20260923.md)。
...
## Decode扫描发现Graph覆盖遗漏：D96/C80单轮恢复

**此前decode变慢不能解释为GPU的固有batch上限。** 单四卡服务TP2×DP2、EP on、DSpark K5，固定每DP engine `max-num-seqs=48`（服务合计D96），在同一个共享8K前缀／1024输出下得到：

| 全服务并发 | Graph最大tokens | 测量方式 | 输出tok/s | Mean TPOT，ms |
| ---: | ---: | --- | ---: | ---: |
| C64 | 192 | 原扫描4批 | 4290.12 | 9.57 |
| C80 | 192 | 原扫描4批 | 1983.99 | 35.86 |
| C96 | 192 | 原扫描4批 | 2366.89 | 35.68 |
| **C80** | **288** | **完整生成预热1轮＋正式1轮** | **6180.75** | **10.06** |

K5目标验证每请求6 tokens、anchor草稿5 queries。每引擎32条需192／160，40条需240／200，48条需288／240；旧192上限没有覆盖40／48条的大batch形状。固定镜像源码确认无匹配Graph时回退非Graph执行。**只增加捕获尺寸200／240／288、把上限改为288后，C80吞吐约提高到3.12倍、TPOT下降72%。** 两DP引擎实测running均达到40，正式80成功／0失败、0已知JIT、0抢占；没有修改forward、精度或预热补丁。

这是**本地高前缀命中的decode参照**：命中96.875%，每请求仍计算256个输入tokens；共享前缀不是独立GovReport请求。旧扫描没有独立完整生成预热，C64首批附近含JIT；旧批次计时取最长单请求，新实验使用真实批次墙钟。新结果只有一轮，未测修正后的C64／C96，也不能将6181直接当作PD吞吐。完整生成预热出现12条已知采样内核事件，正式轮为0，支持继续保留完整HTTP预热。

历史16K PD的D96也沿用了192上限。**历史数字仍有效，但“D96容量太大所以天然变慢”的解释须撤回；补齐Graph后的PD收益待复测。** 优先验证16K 3P1D、D96/C96，再选择容量和配比。见[核查与实测报告](reports/decode-graph-coverage-20260923.md)、[逐批CSV](data/decode-graph-coverage-20260923.csv)及[实测命令](reports/decode-graph-coverage-20260923-commands.md)。
...
### PD容量与配比怎么选：先测独立能力，再做端到端验证

**用 prefill-only／decode-only 缩小P/D拓扑、容量和配比的候选范围，再用完整PD确认系统并发与收益。** 独立能力不是部署性能的直接预测：连接后还有供给波动、传输、排队和资源竞争。本项目已完成本地8K扫描及C80的Graph修正单轮验证，结果见上方；真实16K PD仍须单独复测。

| 步骤 | 测什么 | 用来决定什么 |
| --- | --- | --- |
| 1. Prefill-only | 固定真实输入集，关闭输入前缀复用，输出1个token；记录完成请求/s、输入tok/s、排队和首token延迟 | 在延迟可接受时，每个P能持续供给多少请求；短输出仍有首token计算与HTTP开销 |
| 2. Decode-only | 预先准备KV，分别改变运行上限与客户端C；记录逐DP rank实际running、等待、TPOT、输出tok/s和草稿接受率 | 找出D有吞吐收益且TPOT可接受的实际批量区间，再选`max-num-seqs`候选 |
| 3. 匹配P/D | 将P完成速率与候选批量下D完成速率统一为请求/s；固定输出1024时，D请求/s约为输出tok/s÷1024 | 在固定GPU总数内只选少量相邻配比，服务数及每服务GPU数分别列明 |
| 4. 完整PD验证 | 固定D容量，测少量系统C；同时记录P等待、传输、D运行与等待，以及端到端延迟 | 选goodput、联合SLO达标率和吞吐较好的工作点，并与同资源普通服务比较 |

**扫描客户端C不等于扫描decode batch。** DP2下每引擎`max-num-seqs=32`，服务运行上限合计64；即使C96，运行数仍受此限制。验证更大batch须提高相应上限、核实实际running，并检查Graph与DSpark草稿／验证形状的覆盖。`running`也不是每一步精确的执行batch；定位内核拐点还需调度或profile证据。旧PD D96存在Graph覆盖遗漏；应先补齐执行形状，再判断真实batch性能边界。

容量匹配的初估是`P服务数 × 单P完成请求/s ≈ D服务数 × 单D完成请求/s`，在延迟约束下保留供给余量，不用`P的max-num-seqs相加=D容量`。两侧拓扑可以不同，仍须满足总GPU预算。**系统C不必等于D容量**：它还覆盖P处理、传输和排队中的请求；稳定窗口可用`平均在途数≈完成请求/s×平均E2EL`估算范围，再以实测校准。逐DP rank容量不能跨rank共享。先固定D上限调C，需要验证容量影响时再固定C调D，避免同时改变两个变量。

独立测试须对齐模型、固定镜像、输入／输出长度、数据、采样、DSpark草稿策略与CPU/GPU绑定；8K结论不能直接用于16K。prefix-cache方式还须验证每个DP rank的实际命中与残余prefill，并记录提示词池大小和前缀共享程度；少量重复前缀可能改变KV访问局部性和草稿接受率。其HTTP测量仍可能含排队、残余prefill和首token，必须写清计时边界，不能称为纯GPU计算时间或直接等同NIXL接收KV的路径。

保留既有JIT缓存与启动覆盖，预热和正式测量分开，预算及重复数预先固定。最终普通／PD对照使用相同总GPU、数据、输出长度、系统C和DSpark设置，**Output Token Throughput始终按完整窗口计算**，PD包含代理、P、KV传输和D；decode-only吞吐单独标注。使用逐请求TTFT／TPOT联合SLO计算goodput与达标率，不能只用平均延迟验收。当前C80/D64的87.97%是候选间较好的结果，仍未达到95%联合目标，见[容量报告](reports/pd-16k-c96-3p1d-capacity.md)。

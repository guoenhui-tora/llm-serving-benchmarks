...
三类已知缺口均已修复。修复验证覆盖DSpark 1P1D、DSpark普通双服务及16K普通六TP4：**40个worker检查通过，9轮正式测量全部0已知编译事件**。后续16K C128普通三TP4／2P1D对照中，另有24个worker通过检查、6轮正式测量均0事件。修复验证中新增输入内核覆盖每worker约0.14–0.24秒，适用已继承缓存的配置，不代表冷启动编译成本。
...
3. **单独保存完整HTTP预热，再测三轮quick。** 预热使用与正式测量相同的完整请求集：历史C32／C64分别用128／256条，最新C128用512条；请求量须按实验协议明确指定，不能只由C推定，默认2C预热不能替代。修复验证中的普通DSpark预热仍有24条其他内核事件，正式三轮才为0。
...
## 本轮执行计划：16K、DSpark K5，普通双服务与1P1D的C32／C64对照
...
**执行补记：** PD两档已完成；普通首次启动误继承修复前的输入预热脚本，在请求发送前退出（0条请求），失败产物保留。仅为纠正这一准备错误，增加一次普通服务启动：继承原缓存并覆盖为与本轮PD相同的已验证预热模块，核对哈希后执行。总时间截止、功能／性能请求数及三轮quick均不增加，不再重试。

配置、实际命令和全部原始结果写入45本机`experiments/dsv4-pd-dspark16k-c32c64/`（仅本机可用），运行时显式指定该工作区内的新run-root。此处是执行计划，不是已验证结果。

## 跨节点PD研究

**目前尚未找到可跨负载通用的PD优胜方案。主目标16K输入下，12卡TP4 2P1D在C64／N256曾领先普通2.88%，最新C128／N512则落后6.24%，不建议按吞吐目标直接扩规模。** 8K、C32下的TP2×DP2 EP on DSpark K5 1P1D仍是小幅正向候选（+5.43%）。这些结果均是单次启动内重复，尚未做独立重启复核。
...
### 1. 最新16K对照：C128下12卡2P1D吞吐落后，首token等待明显增加
...
修复后K5配对的延迟（普通→PD）：Mean TTFT为1.763→3.985秒（+126.03%），Mean TPOT为19.491→15.883毫秒（−18.51%），Mean E2EL下降6.77%。吞吐与生成速度提高，但首token等待更长；这一结果尚未扩展到16K。
...
固定镜像内NIXL 1.3.2已完成上述DSV4 FP8、packed MLA、滑动窗口及8K DSpark路径的真实传输验收；完整生成质量与逐token数值等价尚未验证。16K下的DSpark和TP2×DP2等其他P/D拓扑仍未实测。部署过程和时序图见[跨节点协同流程](reports/pd-cross-node-flow.md)。

后续以本页已授权的**16K、TP2×DP2 EP on K5普通／1P1D C32与C64配对**为主线，先验证兼容性及P/D容量匹配，暂不扩大TP4比例矩阵。普通混合推理的最佳拓扑不能直接当作纯P或纯D最优，是否采用PD还需明确首token与尾延迟要求。

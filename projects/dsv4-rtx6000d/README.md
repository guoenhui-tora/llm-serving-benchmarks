...
**同16卡下，3P1D三档吞吐均超过普通四服务，但不能只按最高吞吐选择并发。** 每服务4卡、TP2×DP2 EP on、DSpark K5、budget＝16K；C为全系统并发，D服务容量随三档分别为96／128／144，Graph分别288／384／432。输入为独立GovReport精确16384-token前缀，输出1024；双方每档2C完整生成预热＋三轮各4C请求。吞吐包含代理、P、传输与D，单位output tok/s，均值±样本SD。
...
SLO固定逐请求TTFT≤10s且TPOT≤50ms，goodput为达标请求/完整窗口。**C96的达标率较高；C128的goodput均值略高，但三轮波动尚不能确认其可靠领先C96；从C128继续升至C144只有小幅吞吐增益，却明显损失goodput。** 三档PD的SLO失败全部来自TTFT，未见TPOT>50ms；普通C128则约42%的请求TPOT超标。主线D遥测未见容量等待，P平均排队随C增加而变长，不能把问题归为D容量仍太小。三档均未达到95%联合目标。

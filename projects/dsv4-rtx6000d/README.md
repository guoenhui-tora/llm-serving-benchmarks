...
本项目使用 NVIDIA 发布的 [nvidia/DeepSeek-V4-Flash-0731-NVFP4](https://huggingface.co/nvidia/DeepSeek-V4-Flash-0731-NVFP4)，目标是在 RTX6000D 上优化推理吞吐与延迟，已完成单机拓扑研究及一轮跨节点PD分层筛选，当前结果与后续方向见下文。权重为 NVFP4 routed experts 与高精度其他部分的混合格式；实验保持同一权重与 tokenizer。
...
原TP4 mHC模块、[16K预算／DP／DSpark扩展模块](patches/mhc-startup-warmup-extended/README.md)和新输入内核模块的范围分别记录，不能跨镜像直接套用。原模块解决mHC入口遗漏，新模块补齐top-k布局与DSpark草稿跨度；其他内核仍需HTTP预热。历史事件与本次修复后的数据分批保存。

**没有JIT也不能省完整HTTP预热或稳定性检查。** 夜间DP off的PD预热仅733.26 tok/s、0事件，正式才927.50；8K TP4 2P2D正式三轮0事件却有明显漂移，并伴随预热NIXL长尾。启动覆盖、HTTP预热、真实传输和性能波动须分别验收，详细结果见下节。

## 跨节点PD研究

**PD链路已验证，性能收益取决于输入长度、并发、每服务拓扑和P/D配比。** 当前值得继续确认的两个候选是：近8K、同八卡C32的TP2×DP2 EP on＋DSpark K5 1P1D，以及16K、同12卡C64的TP4 2P1D。两者都有吞吐增益，也都增加首token等待；尚未确定通用最优PD方案。

- **TP4 1P1D没有超过匹配普通推理。** 8K同节点/跨节点均低约11%，16K低约15%–17%；输入变长并不会自动让1P1D受益。
- **增加P更适合本轮16K压力，但扩大规模并非线性提速。** 12卡2P1D比普通高2.88%；16卡2P2D及24卡4P2D均未超过同资源普通。
- **DSpark修复后有独立的无已知事件对照。** 8卡1P1D为1503.25±21.52、普通双服务1425.76±28.85 tok/s，观察增益5.43%；Mean TTFT高126.03%，Mean TPOT低18.51%。需独立重启复核收益与尾延迟，不能仅凭三轮无事件宣布稳定优势。

### 比较口径与数据

**下表吞吐均为全系统output tok/s，均值±样本SD；百分比为`PD/匹配普通−1`。** 每组一次启动、一轮完整HTTP预热、固定三轮quick，全部正式轮保留。0事件不等于stable通过；三轮同次启动也不是三次独立重启。端到端计时包含45代理、P处理、KV传输、D生成与排队，普通同样经过代理，不相加各服务自己的吞吐。

所有服务保持vLLM `v0.29.0`、固定image ID `sha256:c2914767605584b6d8f45686b82de173ecc99e781897aa3d0a66dacd72c51ae1`、同一NVFP4模型及FP8 E4M3 KV；关闭prefix cache和FlashInfer autotune，保留正常Graph与JIT缓存。45统一控制，服务按组使用46/47/48。比较双方使用相同GPU集合、请求集和**全系统**并发，但分阶段的代理、UCX及预热适配有变化，跨阶段不视作单变量实验。

| 负载 | 数据身份 | 全系统并发／每轮请求 | 实际输入tokens／输出tokens |
| --- | --- | --- | --- |
| 近8K＋1K | 原GovReport近8K池，固定前128条 | C32／128 | 1,042,149／131,072 |
| 近8K＋1K | 同池，固定前256条 | C64／256 | 2,090,525／262,144 |
| 精确16K＋1K | 1024个唯一真实报告前缀池，每条16384输入tokens，固定前128条 | C32／128 | 2,097,152／131,072 |
| 精确16K＋1K | 同前缀池，固定前256条 | C64／256 | 4,194,304／262,144 |

1024条是储备池，不是每轮都发送1024条；另有815篇完整近16K报告，它们与本轮精确16K前缀池分开。见[数据准备证据](data/govreport-16k-1024-preparation.json)及[来源与下载](data/govreport-source-download.md)。这些结果不与历史random基线混算，也不把C32/N128与C64/N256合并统计。

**P/D数字始终表示四卡服务数，不表示节点数。** 例如2P1D＝2个P服务＋1个D服务＝3×4＝12卡；4P2D＝6×4＝24卡，放在三台GPU节点，45只做控制。TP2×DP2服务内部的两个DP引擎及API进程不另计P/D服务数。

### TP4 1P1D：位置、输入长度与P预算

夜间组均为普通2服务对照1P＋1D，**每服务TP4、总8卡、DSpark off、C32/N128**；以下各组正式事件均0/0/0。

| 输入／位置 | P预算（tokens） | 普通双TP4 | 1P1D | PD相对普通 |
| --- | ---: | ---: | ---: | ---: |
| 8K，同48前后四卡 | 8192 | 1026.74±1.52 | 911.15±1.36 | −11.26% |
| 8K，46→48后四卡 | 8192 | 1028.71±3.36 | 914.23±1.98 | −11.13% |
| 16K，46→48后四卡 | 8192 | 741.08±0.45 | 615.30±0.09 | −16.97% |
| 16K，46→48后四卡 | 16384 | 741.08±0.45（同上参照） | 631.84±0.07 | −14.74% |

同节点与跨节点结果接近，**目前没有证据把约11%的差距主要归因于跨节点KV通信**。同节点组需要修正UCX可达性，且实际有NIC活动，因此它不是零通信开销或纯CUDA IPC对照。详情见[位置对照及失败反例](reports/pd-overnight-placement-progress-20260921.md)。

P预算是每次调度允许处理的token预算，**不是输入长度，也不是增加P服务数**。8K仅把P预算8192→16384，PD为899.16±19.28，较原PD低1.65%，Mean TTFT高16.68%，因此8K未替换预算。16K同样调整则改善2.69%，但仍落后普通14.74%；后续16K配比采用P预算16384、D/普通8192。见[8K预算复核](reports/pd-budget-8k-20260921.md)与[16K对照](reports/pd-16k-c32-20260921.md)。

16K原1P1D的Mean TTFT为26.565秒，普通7.281秒；P服务平均排队约23.10秒、prefill指标约2.309秒，而D的Mean TPOT反而更低。这个P供给/排队线索支持测试2P1D；不能仅凭输入变长就继续增加D，服务端计时也不等于纯GPU kernel耗时。

### TP4配比：同资源C64对照

本表为夜间12个性能单元，**每服务TP4四卡、DSpark off、C64/N256**。8K的P/D预算均8192；16K的P预算16384，D/普通8192。12卡使用46/47/48后四卡，16卡增加47前四卡，24卡使用三节点前后四卡；同一行普通与PD使用相同GPU集合。

| 输入 | 服务数×每服务GPU＝总GPU | 普通同数量副本 | PD配比／吞吐 | PD相对普通 | 正式事件／限制 |
| --- | --- | ---: | --- | ---: | --- |
| 8K | 3×4＝12 | 1650.78±0.77 | 1P2D：1243.88±0.16 | −24.65% | 两组0/0/0 |
| 8K | 3×4＝12 | 1650.78±0.77 | 2P1D：1346.24±1.66 | −18.45% | 两组0/0/0 |
| 16K | 3×4＝12 | 1157.11±0.46 | **2P1D：1190.46±0.76** | **+2.88%** | 两组0/0/0；TTFT增加 |
| 16K | 3×4＝12 | 1157.11±0.46 | 1P2D：664.33±0.04 | −42.59% | 两组0/0/0；P长排队 |
| 8K | 4×4＝16 | 2042.46±13.85 | 2P2D：1762.04±84.42 | −13.73% | 两组0/0/0；PD有漂移 |
| 16K | 4×4＝16 | 1481.30±1.11 | 2P2D：1250.81±5.98 | −15.56% | 两组0/0/0；TPOT轮间CV5.23% |
| 16K | 6×4＝24 | 1876.76±0.27 | 4P2D：1774.22±7.63 | −5.46% | 普通4/4/0，PD0/0/0；含事件比较 |

**16K 2P1D是TP4下的小幅正向候选，代价是更慢的首token。** Mean TTFT由8.195→9.480秒（+15.68%），Mean TPOT由46.401→41.627ms（−10.29%），Mean E2EL由55.663→52.064秒（−6.47%）。反向1P2D的P中段平均等待约50条、两个D各仅5–6条运行，与P供给不足相符。8K 2P1D却是两P等待0、D约62条运行，说明适合的配比随负载变化；队列采样不是唯一瓶颈的证明。

**扩大到4P2D未复现12卡2P1D的收益。** 固定总C64时，24卡PD每GPU吞吐73.93，普通78.20，12卡2P1D为99.20 tok/s/GPU；这些是固定并发下的资源效率，不能据此推断随并发增长的扩展上限。24卡PD的Mean TTFT虽低21.31%，P95 TTFT却高36.67%，吞吐仍较低。这里P95是各轮P95的均值，不是合并请求分位数。

8K 2P2D三轮1664.92→1803.24→1817.95，虽均0事件，吞吐CV4.79%、TTFT CV约19.5%；预热NIXL传输还出现长尾。全部轮次保留，不能只取后两轮或称已稳定。完整延迟、队列、NIC/CPU/GPU和实际命令见[配比报告](reports/pd-ratios-c64-20260921.md)／[逐轮CSV](data/pd-ratios-c64-20260921.csv)。

top-k修复后，**仅普通六TP4**另做了独立验证：1877.08±0.62 tok/s、正式0/0/0，与夜间普通均值相差约0.017%。这补齐了普通路径的覆盖验证；本轮没有重测4P2D，所以上表保留原配对及事件，不把新普通均值与旧PD拼成“修复后配对”。

### TP2×DP2 EP on与DSpark：单独看修复前后

以下均为近8K、C32/N128、相同46/48后四卡，**普通2服务对照1P＋1D，每服务TP2×DP2 EP on四卡，总8卡**。DSpark K5在P/D两端对称配置，target/draft mHC、三个draft SWA KV group注册、各DP引擎草稿接受和真实KV传输均已检查；不把历史单四卡966 tok/s直接作为八卡参照。

| 批次／配置 | 普通双服务 | 1P1D | PD相对普通 | 正式事件与解释 |
| --- | ---: | ---: | ---: | --- |
| 夜间，DSpark off | 1052.33±60.29 | 927.50±5.75 | 不作稳定收益比 | 两组0/0/0；普通CV5.73%，停止off配比扩展 |
| 夜间，DSpark K5 | 1458.47±5.57 | 1485.79±23.12 | +1.87% | 普通4/0/2、PD4/2/0；含事件quick |
| **输入内核修复后，DSpark K5** | **1425.76±28.85** | **1503.25±21.52** | **+5.43%** | 两组0/0/0；CV分别2.02%／1.43% |

修复后组各三轮平均延迟如下，单位分别为秒和毫秒：

| 模式 | Mean TTFT s | Mean TPOT ms | Mean E2EL s |
| --- | ---: | ---: | ---: |
| 普通双服务 | 1.763 | 19.491 | 21.702 |
| 1P1D | 3.985 | 15.883 | 20.233 |
| PD相对普通 | +126.03% | −18.51% | −6.77% |

这体现了生成阶段更快、首token等待更长的取舍。**5.43%是修复后独立配对的观察值，不能把相对夜间1.87%的变化全部归因于消除JIT**：普通组均值与轮间波动也改变，尚无独立重启重复。两批不合并成六轮；off/on之间还存在代理版本、预热及背景差异，不据此声称严格单变量DSpark增益。见[DP off](reports/pd-dp-off-20260921.md)、[夜间DSpark](reports/pd-dspark-c32-20260921.md)及[修复后验证](reports/input-kernel-warmup-20260921.md)／[CSV](data/input-kernel-warmup-20260921.csv)。

### 已确认边界与后续优先级

固定镜像NIXL 1.3.2下，DSV4 packed MLA、FP8 KV及滑动窗口跨节点传输已实测；上述DSpark配置还验证了draft KV注册与内部DP路径。完成的PD轮次均联合核验P处理全输入、D全部远端命中、真实字节与逐TP传输、D新增prefill计算为0，以及零传输/通知失败、过期和抢占。**HTTP成功或remote hit为正不能独立证明PD成功**：同节点原UCX配置曾在这两项为正时出现0传输字节和rank失败，被验收拒绝。生成质量及完整逐token数值等价仍未完成。

夜间已完成**24个性能单元、72轮正式测量**，另保留3次失败：同节点UCX不可达、DSpark预热2次HTTP断开、NIXL侧通道端口占用；分别取证并做有界修正。后续JIT修复独立完成3组/9轮，另保留一次builder误选的零请求启动失败。夜间18,734条与修复验证2,056条请求分别对账；不把失败或预热混入正式均值，历史报告不追改。

后续讨论优先顺序：

1. **确认两个正向候选的重复性与延迟取舍。** 16K/12卡TP4 2P1D，以及8K/8卡TP2×DP2 EP on K5 1P1D；各自配同资源普通，统一当前预热适配和协议，再判断是否值得扩大。
2. **再考虑16K DSpark和纯P/D拓扑优化。** 当前16K DSpark尚未测，P/D异构拓扑也未系统验证；普通混合推理的最佳拓扑不能直接认定为纯P或纯D最优。
3. **规模研究区分固定并发效率与扩展吞吐。** 本轮C64的4P2D没有总体优势，不直接扩矩阵；若提高并发，双方需同步提高请求量并重新明确预算。3P1D只准备、按队列分支被2P2D替代，未执行，不能列作失败或已验证。

这是后续建议，本次README整合不启动新实验。原始产物仅45本机`experiments/dsv4-pd-overnight/`与`experiments/dsv4-jit-coverage-fix/`；精选数据和复现命令见各报告。当前配置入口尚未自动包含全部PD编排和新worker接入，复用时按报告准备工作区。

### 完整报告与研究过程

| 入口 | 内容 |
| --- | --- |
| [夜间总览](reports/pd-overnight-results-20260921.md)／[执行计划](reports/pd-overnight-plan-20260921.md) | 24组结果、阶段预算、失败与跳过依据；总览保留11:25时点状态 |
| [位置对照](reports/pd-overnight-placement-progress-20260921.md) | 同节点/跨节点、同节点传输失败反例与UCX适配 |
| [8K预算](reports/pd-budget-8k-20260921.md)／[16K C32](reports/pd-16k-c32-20260921.md) | P预算8192/16384及输入长度对照 |
| [C64配比](reports/pd-ratios-c64-20260921.md) | 12/16/24卡普通与PD、延迟/队列/资源、完整命令 |
| [DP off](reports/pd-dp-off-20260921.md)／[DSpark K5](reports/pd-dspark-c32-20260921.md) | TP2×DP2兼容、匹配对照、草稿接受与历史JIT |
| [JIT修复验证](reports/input-kernel-warmup-20260921.md) | 最新三组无已知事件验证、补丁、测试及实际命令 |
| [跨节点启动与协同流程](reports/pd-cross-node-flow.md) | 45控制、P/D各自加载模型、代理时序与NIXL直接传KV的图解 |

<details>
<summary>早期链路验证与普通基线校准（历史结果，不能替代上述对照）</summary>

- [首轮计划](reports/pd-1p1d-plan.md)／[首轮实测](reports/pd-1p1d-20260920.md)：先完成C1真实传输，输出差异阻止直接进入性能结论。
- [普通路径重复性](reports/ordinary-repeatability-20260920.md)：移除connector仍存在长输入输出差异，不能直接归因于PD；完整数值原因仍未定位。
- [快速功能检查](reports/pd-screen-20260920.md)／[补充计划](reports/pd-screen-plan.md)：概率、可核验答案和KV错配负向测试；早期PD747.91/普通682.09均受JIT与波动影响。
- [分流校准](reports/pd-calibration-20260921.md)：修正旧轮询31/1失衡，第7轮1033.08属于普通双TP4，不是1P1D，也不是三轮基线。
- [mHC定向预热对照](reports/pd-targeted-warmup-20260921.md)／[CSV](data/pd-targeted-warmup-20260921.csv)：早期普通1029.25±1.58、1P1D913.91±1.66，各三轮clean；后续位置与拓扑研究沿此继续。
- [16K首分片就绪报告](reports/govreport-near16k-readiness-20260921.md)：历史数据检查；完整1024条主负载准备证据优先于首分片结果。
...
普通单机拓扑、DSpark开关与K扫描、首轮多节点PD分层筛选及已知JIT缺口修复均已完成；历史PARTIAL不自动补测。当前PD候选、未验证项和确认顺序以[PD后续优先级](#已确认边界与后续优先级)为准，具体实验另定预算。

- 已完成固定镜像Triton缓存持久化验证，以及mHC、top-k、DSpark草稿输入的定向启动覆盖；新配置继续检查各worker、完整HTTP预热和日志事件，不假设所有分派已经覆盖。
- PD优先确认16K/12卡TP4 2P1D与8K/8卡TP2×DP2 EP on K5 1P1D的收益及首token代价，再考虑16K DSpark和分别优化P/D拓扑。
- 普通双TP2×PP2、双TP2×DP2 EP on的波动，以及vLLM `--numa-bind`与Docker cpuset的worker/页面分布比较，保留为独立方向，不与PD结论混算。

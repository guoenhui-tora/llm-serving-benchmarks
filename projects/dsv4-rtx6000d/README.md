...
D两档中段平均均约19条运行，单DP引擎最大12条，容量等待为0；P平均排队从5.55秒增至29.63秒。此处证据指向P供给／调度，未支持提高D运行上限。固定镜像K5预留4个草稿输入槽，16K预算会把精确16384输入拆成`16380+4`两步；本轮已用功能追踪确认，但A组吞吐与此前同预算结果一致，尚无证据表明这4个槽是主要瓶颈。尝试将P预算提高到16640时，另一层固定mHC预热仍拒绝超过16384，未产生性能数据。详见[DSpark预算与四槽验证](reports/pd-dspark-budget-20260922.md)。

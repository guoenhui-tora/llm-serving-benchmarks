...
- [x] **47**：正式整机样本30次（同启动完整C32/C64配置24次＋TP4×DP2 EP off上午C32、下午C64各独立接纳三次）。C32为679.75 ± 1.37 tok/s；C64为878.47 ± 26.48 tok/s、CV 3.01%，保留波动限制。两组来自不同启动，上午原case仍FAIL；不宣称同启动矩阵完成。见[本节点报告](reports/node8-node47.md)、[逐次CSV](data/node8-node47.csv)。

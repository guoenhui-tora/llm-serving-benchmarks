...
## 2026-09-25：vLLM 0.30，24 卡 5P1D/C144 对照 4P2D

用相同 24 卡、真实独立 GovReport 16K/1024、全局 C144 单独测了五个四卡 P＋一个四卡 D（全部 TP2×DP2、EP on、DSpark K5）。5P1D 三轮整机 **3452.64±35.25 output tok/s**，比先前 4P2D/C144 的 **4120.98±8.80** 低 **16.22%**；Mean TTFT 从 **9.190 降至 6.426 s**，但 Mean TPOT 从 **23.516 升至 32.851 ms**，goodput 从 **3.167 降至 2.784 req/s**。多一个 P 改善了首 token，少一个 D 则损失吞吐/完成时间；不能仅凭 TTFT 断言 5P1D 更适合本负载。这是不同启动批次且路由/同节点路径改变的单档对照，没有扩为配比矩阵。[完整逐轮结果、阶段耗时、命令与复现边界](reports/pd-24gpu-16k-5p1d-c144-20260925.md)。

## 2026-09-25：vLLM 0.30，24 卡四 P＋单八卡 TP2×DP4 D 的 C144 对照

保持 46/47 的四个四卡 P 不变，只将 48 上两个独立四卡 D 改为一个八卡 TP2×DP4 D；相同整机名义容量 256、真实 GovReport 16K/1024 和全局 C144。单八卡 D 三轮 **3769.06±65.07 output tok/s**，比原 4P2D/C144 的 **4120.98±8.80** 低 **8.54%**；Mean TTFT **8.223 vs 9.190 s** 有改善，但 Mean TPOT **28.038 vs 23.516 ms**、goodput **2.991 vs 3.167 req/s** 均变差。远端 KV、四个 D 引擎、Graph 与三轮完整工作量已验收；单八卡 D 不符合“同时提高吞吐与降低 TTFT”的目标。decode-only 的赢家原本也是**双四卡 D**，本次补齐真实 PD 端到端证据，不扩并发。[逐轮结果、实际命令与验收证据](reports/pd-24gpu-16k-single-8gpu-d-c144-20260925.md)。

## 2026-09-25：vLLM 0.30，24 卡 4P2D 的 16K/1024 并发扫描

在 46/47/48 各八卡固定四个四卡 P、两个四卡 D（各 TP2×DP2、EP on、DSpark K5），真实 GovReport 精确 16K/1024、全局 C16～C144 步长 16 以及追加 C160 均完成完整预热与三轮正式测量。整机 output tok/s：C64 **2680.39±14.17**、C128 **3930.44±18.39**、C144 **4120.98±8.80**；原扫描 C160 为 **4263.74±35.61**，Mean TTFT **11.898 s** 越过 10 s 停扫线。**另两次独立启动**分别补测 C148、C152，并已放入原并发表格：C148 **4167.59±12.15 tok/s**、Mean TTFT **9.687±0.263 s**、goodput **3.114±0.081 req/s**，三轮均值 TTFT 均低于 10 s；C152 **4203.64±5.28 tok/s**、Mean TTFT **10.705±0.560 s**、goodput **2.836±0.289 req/s**，三轮均值 TTFT 均越线。已测门槛边界是 C148/C152，C145～C147、C149～C151 未测；C148 吞吐虽高于 C144，但离均值门槛仅约 0.313 s，逐请求达标率 76.52%，不称为线上最优。按**相同全局并发/GPU**对齐历史，12卡2P1D/C64→本批24卡4P2D/C128 吞吐约2.02倍，16卡3P1D/C96→本批C144约1.60倍；旧组镜像、数据和容量不同，这只是方向性历史比例，不证明 4P2D 对 16K 最优。同资源 `2×(2P1D)` 未测。双四卡 D 在八卡 decode-only 对照胜出，因此按预定规则没有运行单八卡 D 的 PD 拓扑。[完整并发表格、两档补测和复现方法](reports/pd-24gpu-16k-4p2d-20260925.md)。
...
1. **保留缓存，核对补丁。** 新recipe继承已有JIT缓存，逐节点检查缓存内native MXFP4草稿分派、DP profiling回移及预热模块哈希。旧缓存不一定带最新补丁；缺profiling回移曾再次触发`16384 vs 160`断言。
2. **完成每个worker的定向覆盖。** 组合[mHC扩展](patches/mhc-startup-warmup-extended/README.md)与[top-k／DSpark输入覆盖](patches/input-kernel-warmup/README.md)，使用`--worker-cls dsv4_input_worker.Worker`。每worker均检查两遍cache key不增加、有限值正确、临时allocated显存恢复；PD另检查draft KV注册。当前DSpark有界启动覆盖已扩至576，固定镜像测试及D192真实启动通过，修复提交`c210a41`。
3. **检查实际Graph捕获，再做独立HTTP预热。** 参数、K和每rank容量改变后重新计算覆盖；启动初期用于显存估算的2/2捕获不代表最终Graph集合。本轮16K协议为1轮**2C条完整长度生成预热**＋3轮各4C正式测量；历史部分实验用完整N预热，仍按各报告解释。2C和N都不保证覆盖所有调度形状，必须保存正式事件与波动。
4. **固定正式三轮，核对真实工作量。** 不挑最快、不因JIT追加到满意；0事件也不保证稳定。本轮D192七档共21轮正式0已知事件，仍保留约0.84%–2.63%的吞吐CV和后台CPU影响。
...
| C／D容量 | 普通四个完整服务，共16卡：tok/s | 3P1D，共16卡：tok/s | PD变化 | 普通TTFT／TPOT | PD TTFT／TPOT | goodput，普通→PD req/s | 联合SLO，普通→PD |
| ---: | ---: | ---: | ---: | --- | --- | --- | --- |
| 96／96 | 2192.03±27.08 | 2582.57±22.28 | +17.82% | 5.201s／37.226ms | 8.790s／26.132ms | 1.595±0.048→2.001±0.019 | 74.48%→79.34% |
| 104／112 | 2256.61±19.32 | 2721.58±30.78 | +20.60% | 5.487s／39.209ms | 9.466s／26.298ms | 1.514±0.058→2.091±0.027 | 68.67%→78.69% |
| 112／112 | 未测 | 2815.23±18.83 | — | 未测 | 10.272s／26.668ms | —→2.107 | —→76.64% |
| 128／128 | 2395.74±2.68 | 3067.25±9.80 | +28.03% | 6.363s／45.646ms | 11.722s／27.371ms | 1.016±0.027→2.106±0.125 | 43.42%→70.31% |
| 144／144 | 2454.89±8.83 | 3183.15±2.80 | +29.67% | 6.771s／50.607ms | 14.161s／28.057ms | 0.745±0.003→1.011±0.156 | 31.08%→32.52% |


SLO固定逐请求TTFT≤10s且TPOT≤50ms，goodput为达标请求/完整窗口。**C96的达标率较高；C128的goodput均值略高，但三轮波动尚不能确认其可靠领先C96；从C128继续升至C144只有小幅吞吐增益，却明显损失goodput。** 五档PD的SLO失败全部来自TTFT，未见TPOT>50ms；普通C128则约42%的请求TPOT超标。主线D遥测未见容量等待，P平均排队随C增加而变长，不能把问题归为D容量仍太小。五档均未达到95%联合目标。

**C104/D112是目前已测3P1D中，满足Mean TTFT＜10秒且吞吐最高的工作点。** 相对同16卡普通四服务，PD吞吐高20.60%、goodput高38.17%，联合SLO从68.67%升至78.69%；代价是Mean TTFT从5.487秒增至9.466秒，TPOT则从39.209ms降至26.298ms。它适合作为当前候选，但没有达到95%逐请求SLO目标。两组同数据、GPU集合、C104及208条预热＋三轮各416条正式，均0失败、0已知JIT；独立批次并保留CPU背景限制。见[普通与PD完整对照](reports/ordinary-vs-pd-c104-20260923.md)。

C112/D112吞吐略高，但三轮Mean TTFT均超过10秒；固定D112降至C104后，吞吐仅减少3.33%，三轮Mean TTFT为9.516／9.275／9.606秒。详见[C112补测](reports/pd-16k-3p1d-c112-20260923.md)与[C104补测](reports/pd-16k-3p1d-c104-d112-20260923.md)。C104/D112目前仍未达到3000 tok/s，继续优化应与已确认的收益和延迟代价区分。

### 同协议复现Graph问题

固定16卡3P1D、C96/D96和请求协议，只改变D的Graph捕获配置：

| D Graph最大tokens | 输出tok/s | Mean TTFT／TPOT | goodput | 联合SLO |
| ---: | ---: | --- | ---: | ---: |
| 192，旧配置 | 1480.51±48.58 | 7.224s／55.114ms | 0.337 req/s | 23.26% |
| 288，覆盖每rank48条K5 | 2582.57±22.28 | 8.790s／26.132ms | 2.001 req/s | 79.34% |

**本轮Graph288比Graph192吞吐高74.44%、TPOT低52.59%；两组正式轮均0已知JIT，支持Graph覆盖不足是此前失速的重要原因。** 旧组KV池峰值19.32%、无抢占和整段输入重算，草稿接受率与新组接近。旧组吞吐1429→1486→1526逐轮上升，CV3.28%，不称stable结果；每轮仍明显落后。两组顺序运行、控制机背景不完全相同，完整边界见报告。历史“D96天然比D64慢”的解释应撤回。

### 供给补测：4P1D与同20卡普通服务

增加一个四卡P后，PD为**4个P＋1个D＝5服务×4卡＝20卡**，与同20卡普通五服务比较。两档固定D144/Graph432，只调整全系统C及协议要求的2C预热、4C正式请求量。
...
C144下，3P→4P把TTFT从14.16降到9.18s，goodput从1.011增到2.574 req/s；GPU多25%，吞吐只增5.98%。D实际批量增大后TPOT从28.06升到30.93ms，抵消部分首token收益。增加P改善了等待，但并不按P数量线性增加吞吐；跨16/20卡的数字只用于机制分析，同资源收益见表。固定D144时，C128比C144达标率仅高约1.11个百分点，goodput反而约低4.73%，未解决95%达标目标；两档是吞吐与平均延迟的取舍。
...
| C | output tok/s，均值±SD | Mean TPOT，ms |
| ---: | ---: | ---: |
| 32 | 1685.82±33.36 | 17.77 |
| 64 | 2618.75±68.90 | 22.92 |
| 80 | 2948.41±61.15 | 25.40 |
| 96 | 3292.18±44.12 | 27.32 |
| 128 | 3811.80±42.97 | 31.34 |
| 144 | 4033.63±33.81 | 33.14 |
| 192 | 4646.83±96.51 | 37.91 |

**没有复现C64→C96的TPOT突然倍增；截至C192吞吐仍在增长，尚未测出硬件极限。** 两rank实测running分别达到96，KV池峰值18.84%，21轮正式0已知JIT、0失败、0抢占。C192联合SLO98.96%，少量长尾与单rank局部容量等待同窗出现；总C等于总容量不保证每个rank都无排队。

每条请求复用同一个真实16K前缀，命中98.4375%，仍计算256个输入tokens；HTTP窗口也包含调度和剩余prefill。其K5接受率约43%，独立GovReport PD约47%，旧8K共享内容约90%，不能把这些吞吐直接相除当作输入长度成本或PD效率。报告、数据身份和实际命令见[完整扫描](reports/decode-16k-d192-20260923.md)。


### 8K历史线索

此前D96/C80、Graph192的8K共享前缀参照约1984 tok/s，补到288后单轮约6181 tok/s，首次暴露Graph覆盖遗漏。两批预热与计时方式有差别，只作历史线索；本轮16K已另做统一协议的PD Graph对照，见上方PD表。[原始核查与8K单轮](reports/decode-graph-coverage-20260923.md)保留旧C64/C80/C96数字和边界，不与本轮16K统计合并。

## 8卡历史对照：16K、DSpark K5，普通与1P1D的C32／C64
...
## 12卡历史对照：16K、TP2×DP2 EP on、DSpark K5
...
D=64/C96 组两个 DP engine 都达到 `running=32`，每个 engine 有2.76–4.38条平均 `capacity` waiting、峰值11–14；C80 时降为约0.69–1.52条、峰值4–6。D=96 组 capacity waiting 降为0–0.06，running均值约37、峰值48。两组KV峰值仅17%／19%，每轮NIXL 1536次传输全部成功，D端输入KV命中12,582,912 tokens且computed-prefill为0。C80 在几乎不牺牲吞吐的情况下显著降低了 C96 的排队和 TTFT；按当批 TTFT≤10s、TPOT≤50ms 目标，C80 是旧D64配置下较好的已测候选。扩大D容量减少了序列上限等待，但旧Graph最大192未同步扩展；因此这组D96的高TPOT不能单独归因于容量变大。D64/C80保留为旧配置下的已测候选；补齐Graph后的16K D96已完成同协议PD对照，见上方本轮结果，不能继续沿用“D96不适合”的建议。完整证据见[16K C96容量报告](reports/pd-16k-c96-3p1d-capacity.md)。
...
**用 prefill-only／decode-only 缩小P/D拓扑、容量和配比的候选范围，再用完整PD确认系统并发与收益。** 独立能力不是部署性能的直接预测：连接后还有供给波动、传输、排队和资源竞争。本项目已完成16K固定D192七档参照、同协议Graph对照、16卡3P1D与20卡4P1D配对，见上方；独立参照仍不能代替完整PD验证。
...
**扫描客户端C不等于扫描decode batch。** DP2下每引擎`max-num-seqs=32`，服务运行上限合计64；即使C96，运行数仍受此限制。验证更大batch须提高相应上限、核实实际running，并检查Graph与DSpark草稿／验证形状的覆盖。`running`也不是每一步精确的执行batch；定位内核拐点还需调度或profile证据。旧PD D96存在Graph覆盖遗漏，本轮已完成同协议修正对照；扩大容量前必须先补齐执行形状，再判断真实batch性能边界。
...
保留既有JIT缓存与启动覆盖，预热和正式测量分开，预算及重复数预先固定。最终普通／PD对照使用相同总GPU、数据、输出长度、系统C和DSpark设置，**Output Token Throughput始终按完整窗口计算**，PD包含代理、P、KV传输和D；decode-only吞吐单独标注。使用逐请求TTFT／TPOT联合SLO计算goodput与达标率，不能只用平均延迟验收。历史C80/D64的87.97%采用N768协议，与本轮N=4C的初始填充占比不同，不能直接混合排名；原结果见[容量报告](reports/pd-16k-c96-3p1d-capacity.md)。
...
本轮已经确认Graph修正后的16卡3P1D超过同资源普通服务；下一步重点从“D96是否天然变慢”转为**真实输入下P供给、D批量与首token等待如何匹配**。三P主线的高并发损失主要体现为TTFT，增第四个P能减轻等待，但需要额外GPU，且更大D批量会增加TPOT。不能据本地共享前缀decode最高吞吐推定还应不断加P或提高C。

后续应先确定业务是否需要承受一次性首批C突发。当前rate=inf、N=4C结果完整保留；若目标是长时间交互服务，再单独约定更长的持续负载或到达速率协议，验证候选的逐请求SLO与尾延迟，不通过删除首批来改善历史分数。本轮已补固定D144的C128／C144，降低并发只小幅改善达标率，未提高goodput；先明确业务负载与验收目标，再决定下一项验证，不继续铺更多P/D比例。

2P2D历史配对的goodput和联合SLO较差，暂不扩大4P2D。纯P/D的最佳拓扑也仍未独立确定，普通混合推理候选不能直接视作最优。旧缓存缺DP profiling overlay导致的断言、同节点DP端口冲突等准备问题已修正并保留证据，见[前一轮执行记录](reports/pd-night-20260922-execution.md)。

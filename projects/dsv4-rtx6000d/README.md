...
**同16卡下，已配对的C96／128／144三档3P1D吞吐均超过普通四服务，但不能只按最高吞吐选择并发。** 每服务4卡、TP2×DP2 EP on、DSpark K5、budget＝16K；C为全系统并发，D服务容量96／112／128／144分别对应Graph288／336／384／432，具体配对见表。C104/D112与C112/D112为后续仅PD补测，普通服务未测；C104沿用D112/Graph336。输入为独立GovReport精确16384-token前缀，输出1024；双方每档2C完整生成预热＋三轮各4C请求。吞吐包含代理、P、传输与D，单位output tok/s，均值±样本SD。
...
| 104／112 | 未测 | 2721.58±30.78 | — | 未测 | 9.466s／26.298ms | —→2.091 | —→78.69% |
...
**C112/D112补测仍未达到“约3000 tok/s且Mean TTFT＜10秒”。** 按同协议完成224条预热＋三轮各448条正式，吞吐2815.23±18.83 tok/s，三轮Mean TTFT为10.254／10.133／10.429秒；goodput约2.107 req/s，与C128基本持平，联合达标率76.64%。本次1344成功、0失败、正式0已知JIT，D整段输入重算0。C112是独立批次，且C/D/Graph/N随档位改变，表中是部署工作点对比；没有普通C112结果，不计算该档PD收益。见[补测报告与四档数据](reports/pd-16k-3p1d-c112-20260923.md)。

**固定D112再降至C104：2721.58±30.78 tok/s，Mean TTFT 9.466秒。** 保持全部P/D参数，208条完整预热＋三轮各416条正式；Mean TPOT 26.298ms、goodput 2.091 req/s、联合SLO 78.69%。该档达到Mean TTFT＜10秒，但吞吐仍未达到3000 tok/s；平均延迟与逐请求SLO分别判断。正式1248成功、0失败、0已知JIT，D整段重算0，普通C104未测。见[补测报告与五档对比](reports/pd-16k-3p1d-c104-d112-20260923.md)。

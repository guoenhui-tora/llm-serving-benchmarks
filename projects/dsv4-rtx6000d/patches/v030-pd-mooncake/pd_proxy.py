"""Experimental Mooncake proxy for TP2/PP2/DP1 producers; no retries or fallback."""
...
def mooncake_registration(value):
    if not isinstance(value, dict) or len(value) != 1:
        raise ValueError('Expected exactly one producer DP engine')
    engine = next(iter(value.values()))
    if not engine.get('engine_id') or set(engine.get('worker_addr', {})) != {'0','1'}:
        raise ValueError('Expected registered TP2 producer')
    if any(set(v) != {'0','1'} for v in engine['worker_addr'].values()):
        raise ValueError('Expected both PP stages for each TP rank')
...
def make_app(prefill, decode, ordinary=(), timeout=180, ordinary_policy='round-robin'):
    ...
    async def lifecycle(app):
        ...
        app['mooncake_sources'] = {}
        try:
            mapping = json.loads(os.environ['MOONCAKE_BOOTSTRAP_MAP'])
            if set(mapping) != set(prefill):
                raise ValueError('Bootstrap endpoints differ from prefill pool')
            for url, bootstrap in mapping.items():
                async with app['client'].get(bootstrap + '/query') as r:
                    r.raise_for_status()
                    engine = mooncake_registration(await r.json())
                app['mooncake_sources'][url] = (bootstrap, engine)
            yield
        finally:
        ...
        try:
            ...
            if ordinary:
                ...
                try:
                    ...
                    p_body = prefill_request(body)
                    p_body['kv_transfer_params']['transfer_id'] = transfer_id
                    async with app['client'].post(prefill_destination + '/v1/completions', json=p_body, headers=headers) as p:
                        p.raise_for_status()
                        payload = await p.json()
                    bootstrap, engine = app['mooncake_sources'][prefill_destination]
                    body['kv_transfer_params'] = {'do_remote_prefill': True,
                        'do_remote_decode': False, 'remote_engine_id': engine,
                        'remote_bootstrap_addr': bootstrap, 'transfer_id': transfer_id}
                finally:
